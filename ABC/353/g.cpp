...
S op(S a, S b){ 
    ll nridx,nrmax,nlidx,nlmax;
    if(a.ridx*c+a.rmax>b.ridx*c+b.rmax){
        nridx = a.ridx;
        nrmax = a.rmax;
    }else{
        nridx = b.ridx;
    ...
    if(-a.lidx*c+a.lmax>-b.lidx*c+b.lmax){
        nlidx = a.lidx;
        nlmax = a.lmax;
    }else{
        nlidx = b.lidx;
    ...
    return {nridx, nrmax, nlidx, nlmax};
}
S e(){ 
    return {0, -INF, 0, -INF};
}
...
int main() {
    ...
    ll m;cin>>m;
    ll ans = 0;
    vector<S> tmp(n, {0, -INF, 0, -INF});
    tmp[0] = {0, 0, 0, 0};
    rep(i,1,n){
        tmp[i] = {i, -INF, i, -INF};
    }
    LazySegmentTree<S, op, e, F, mapping, composition, id> seg(tmp);
    rep(i,0,m){
        ll t,p;cin>>t>>p;
        t--;
        ll num;
        S xr = seg.prod(0, t);
        S xl = seg.prod(t, n);
        num= max(xr.rmax-c*(t-xr.ridx),xl.lmax-c*(xl.lidx-t));
        chmax(ans, num+p);
        seg.apply(t, num+p);
    }

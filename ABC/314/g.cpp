...
int main() {
    ...
    set<pll> stlow, sthigh;
    rep(i,0,m)stlow.insert({0,i});
    vector<ll> dsum(m,0);
    vector<ll> low(m,1);
    ll hp = h;
    LazySegmentTree<S,op,e,F,mapping,composition,id> seg(vector<ll>(m+1,0));
    rep(i,0,n){
        if(low[b[i]]){
            stlow.erase({dsum[b[i]],b[i]});
            dsum[b[i]] += a[i];
            hp -= a[i];
            if(sthigh.size() > 0 && dsum[b[i]] > (*sthigh.begin()).first){
                low[b[i]] = 0;
                pll tmp = *sthigh.begin();
                sthigh.erase(sthigh.begin());
                low[tmp.second] = 1;
                stlow.insert(tmp);
                sthigh.insert({dsum[b[i]],b[i]});
                hp += dsum[b[i]] - tmp.first;
            }
            else{
                stlow.insert({dsum[b[i]],b[i]});
            }
            while(hp <= 0){
                auto it = stlow.rbegin();
                pll tmp = *it;
                low[tmp.second] = 0;
                hp += tmp.first;
                stlow.erase(tmp);
            ...
            seg.apply(sthigh.size(),m+1,i+1);
        }
        else{
            sthigh.erase({dsum[b[i]],b[i]});
            dsum[b[i]] += a[i];
            sthigh.insert({dsum[b[i]],b[i]});
            seg.apply(sthigh.size(),m+1,i+1);
        }
    }
    rep(i,0,m+1) cout << seg.get(i) << " ";
    cout << endl;
    return 0;
}
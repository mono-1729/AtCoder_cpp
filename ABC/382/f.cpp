...
int main() {
    ll h, w, n; cin >> h >> w >> n;
    vector<tuple<ll, ll, ll, ll>> rcl(n);
    rep(i,0,n){
        ll r, c, l; cin >> r >> c >> l;
        rcl[i] = {r, c, l, i};
    }
    sort(rcl.begin(), rcl.end(), greater<tuple<ll, ll, ll, ll>>());
    vector<ll> ans(n);
    LazySegmentTree<S, op, e, F, mapping, composition, id> seg(w+1);
    seg.apply(0, w+1, h);
    for(auto [r, c, l, i] : rcl){
        ll floor = seg.prod(c, c+l);
        ans[i] = floor;
        seg.apply(c, c+l, floor-1);
    }

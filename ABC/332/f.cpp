...
S e(){ return INF; }
S mapping(F f, S x){ 
	return {(x*f.num+f.k)%MOD};
}
F composition(F f, F g){ 
	return {(f.num*g.num)%MOD, (f.num*g.k+f.k)%MOD};
}
F id(){ return ID; }
// ll n;
// vector<S> v(n);
...
int main() {
	...
	vector<S> w(n);
	rep(i,0,n) cin>>w[i];
	LazySegmentTree<S, op, e, F, mapping, composition, id> seg(w);
	rep(i,0,m){
		ll l,r,x;cin>>l>>r>>x;
		seg.apply(l-1,r,{((r-l)*powMod(r-l+1,MOD-2))%MOD,(x*(1*powMod(r-l+1,MOD-2)))%MOD});
	}
	rep(i,0,n){
		cout<<seg.get(i)<<" ";
	}

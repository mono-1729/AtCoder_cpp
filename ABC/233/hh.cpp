...
int main() {
    ...
    vector<ll> x(n), y(n);
    rep(i,0,n) x[i] = pos[i].first, y[i] = pos[i].second;
    WaveletMatrix wm(y);
    ll q; cin >> q;
    rep(i,0,q){
        ll a_, b_, k; cin >> a_ >> b_ >> k;
        ll a = a_+b_, b = a_-b_+M;
        ll ng = -1, ok = MX;
        while(ok-ng > 1){
            ll m = (ok+ng)/2;
            ll l = lower_bound(x.begin(), x.end(), a-m) - x.begin();
            ll r = upper_bound(x.begin(), x.end(), a+m) - x.begin();
            if(wm.range_freq(l,r,b+m+1)-wm.range_freq(l,r,max(0LL,b-m)) >= k) ok = m;
            else ng = m;
        }
        cout << ok << endl;
    }

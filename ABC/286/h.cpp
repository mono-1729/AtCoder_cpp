...
int main() {
    ...
    rep(i, 0, n+2) cin >> points[i];
    Point s = points[n], t = points[n+1];
    Polygon ps = convex_hull(points);
    ll flg = false;

    rep(i,0,n){
        flg |= is_intersect(Segment(s, t), Segment(points[i], points[(i+1)%n]));
        // cout << points[i] << " " << ps[(i+1)%n] << endl;
        // cout << is_intersect(Segment(s, t), Segment(ps[i], ps[(i+1)%n])) << endl;
    }
    if(!flg){
        cout << dist(s, t) << endl;
        return 0;
    ...
    double d1 = 0, d2 = 0;
    for(int i = sid; i != tid; i = (i+1)%ps.size()){
        d1 += dist(ps[i], ps[(i+1)%ps.size()]);
    }
    for(int i = tid; i != sid; i = (i+1)%ps.size()){
        d2 += dist(ps[i], ps[(i+1)%ps.size()]);
    }
    cout << min(d1, d2) << endl;
    return 0;
}
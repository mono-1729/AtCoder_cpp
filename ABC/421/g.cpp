...
int main() {
    ll n, m; cin >> n >> m;
    vector<ll> a(n),vec(n+1);
    rep(i,0,n) cin >> a[i];
    vec[0] = a[0];
    rep(i,1,n) vec[i] = a[i]-a[i-1];
    vec[n] = -a[n-1];
    ll s = n+1, t = n+2;
    MinCostFlow mcf(n+3);
    ll sum = 0;
    rep(i,0,n){
        if(vec[i] > 0) mcf.add_edge(s,i,vec[i],0);
        if(vec[i] < 0) mcf.add_edge(i,t,-vec[i],0), sum += -vec[i];
    } 
    mcf.add_edge(s,n,IINF,0);
    rep(i,0,m){
        ll l, r; cin >> l >> r;
        l--;
        mcf.add_edge(r,l,IINF,1);
    }
    ll res = mcf.min_cost_flow(s,t,sum);
    cout << res << endl;
    return 0;
}
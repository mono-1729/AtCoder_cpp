...
int main() {
    ...
    ll n,l,d;cin>>n>>l>>d;
    vector<S>num (n+d+1, {0, 1});
    num[0] = {1, 1};
    LazySegmentTree<S, op, e, F, mapping, composition, id> seg(num);
    rep(i,0,n+1){
        double x= seg.get(i).value;
        if(i<l){
            seg.apply(i+1,i+d+1,x/d);
            seg.apply(i, i+1, -x);
        }
    }
    vector<double> ans(n+d+1,0),win(n+d+1,0);
    rep(i,0,n+1){
        win[i] = seg.prod(0, i).value+seg.prod(n+1, n+d+1).value; 
    }
    double x = 0;
    rrep(i,n,0){
        ans[i] = max(win[i], x/d);

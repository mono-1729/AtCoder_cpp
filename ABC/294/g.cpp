...
vector<vector<pii>> edges(200005);
vector<pii> edge_to_index(200005),depth(400005,{0,0});
vector<int> check(200005,0),depth(400005,0),dist(400005);

int cnt=0;
void dfs(int pos,int d){
	check[pos]=1;
    for(auto npos: edges[pos]){
        if(check[npos.first] == 0) {
			cnt++;
			depth[cnt]={d+1,cnt};
			dist[cnt]=G[npos.second][2];
			edge_to_index[npos.second].first={cnt};
			dfs(npos.first,d+1);
			depth[cnt]={d,cnt};
			dist[cnt]=-G[npos.second][2];
			edge_to_index[npos.second].second={cnt};
		}
    }
	cnt++;
}
...
int main() {
	...
	dfs(0,0);
	LazySegmentTree<S, op, e, F, mapping, composition, id> lst(depth);
	BIT bit(2*n);

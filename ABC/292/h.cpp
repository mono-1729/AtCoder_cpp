...
int main() {
    ...
    vector<ll> a(n);
    rep(i,0,n)cin>>a[i];
    LazySegmentTree<S, op, e, F, mapping, composition, id> seg(vector<S>(n, 0));
    rep(i,0,n)seg.apply(i,n,a[i]-b);
    while(q--){
        //rep(i,0,n)cout<<seg.prod(i,i+1)<<" ";
        cout<<endl;
        ll c,x;cin>>c>>x;
        c--;
        seg.apply(c,n,x-a[c]);
        ll right = seg.max_right<fx>(0);
        right =min(right+1,n);
        //cout<<right<<" "<<seg.prod(0,right)<<endl;
        cout<<0.0+b+seg.prod(right-1,right)/(right+0.0)<<endl;
        a[c] = x;
    }
    return 0;
}
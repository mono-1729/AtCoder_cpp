...
int main() {
    ll n; cin >> n;
    vector<ll> a(n);
    vector<vector<ll>> idxa(n+1);
    rep(i,0,n) cin >> a[i], idxa[a[i]].push_back(i);
    ...
    vector<S> vec(n);
    rep(i,0,n) vec[i] = numl[i]+numr[i];
    LazySegmentTree<S,op,e,F,mapping,composition,id> seg(vec);
    ll ans = 0;
    rep(i,0,n){
        ll idx = lower_bound(all(idxa[a[i]]), i+1) - idxa[a[i]].begin();
        ll r = idx == idxa[a[i]].size()? n: idxa[a[i]][idx];
        seg.apply(i,r,-1);
        chmax(ans,numl[i] + seg.prod(i+1,n));
    }
    cout << ans << endl;
    return 0;
}
...
int main() {
	...
	ll ans=0;
	vector<S> v(2LL*1e5+1,0);
	LazySegmentTree<S, op, e, F, mapping, composition, id> seg(v);
	ll index=0;
	rep(i,0,200001){
		while(!pq.empty()&&pq.top().first<=i){
			seg.apply(max(pq.top().second-w,0LL),pq.top().second,-1);
			pq.pop();
		}
		while(index<n&&tx[index].first<=i+d){
			pq.push({tx[index].first,tx[index].second});
			seg.apply(max(tx[index].second-w,0LL),tx[index].second,1);
			index++;
		}
		chmax(ans,seg.prod(0,200001));
	}
	cout<<ans<<endl;

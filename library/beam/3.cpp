...
struct Problem {
    ...
    array<Move, 3> moves;
    vector<array<int, 3>> next_pos, prev_pos;
    vector<array<int, 4>> neighbors;
    vector<int> target_time;
    vector<double> weight;
    vector<fast_beam::StateKey> cell_key, pos_key;
};

void prepare(Problem& problem) {
    const int n = problem.n;
    problem.total = n * n;
    // 大きく移動しつつ、選択によって着地点を少しずつ調整できる3本。
    problem.moves = {{{70, 75}, {71, 76}, {72, 75}}};
    problem.next_pos.resize(problem.total);
    problem.prev_pos.resize(problem.total);
    problem.neighbors.resize(problem.total);
    problem.target_time.resize(problem.total);
    problem.weight.resize(problem.total);
    problem.cell_key.resize(problem.total);
    problem.pos_key.resize(problem.total);

    mt19937_64 rng(0x70b0a5e123456789ULL);
    for (int p = 0; p < problem.total; ++p) {
        const int x = p / n, y = p % n;
        problem.neighbors[p] = {x ? p - n : -1, x + 1 < n ? p + n : -1,
                                y ? p - 1 : -1, y + 1 < n ? p + 1 : -1};
        for (int a = 0; a < 3; ++a) {
            const auto [dx, dy] = problem.moves[a];
            problem.next_pos[p][a] = ((x + dx) % n) * n + (y + dy) % n;
            problem.prev_pos[p][a] = ((x - dx + n) % n) * n +
                                      (y - dy + n) % n;
        }
        problem.weight[p] = sqrt(double(p + 1));
        problem.cell_key[p] = {rng(), rng()};
        problem.pos_key[p] = {rng(), rng()};
        const auto [tx, ty] = problem.targets[p];
        problem.target_time[tx * n + ty] = p;
    }
}
...
public:
    ...
    explicit State(const Problem& problem)
        : problem_(&problem), visited_(problem.total, 0),
          nearest_(problem.total, 255) {
        changes_.reserve(problem.total * 3);
        frames_.reserve(problem.total);
        queue_.reserve(problem.total);
    }

    pair<Evaluation, fast_beam::StateKey> initial_node() const {
        return {{}, problem_->pos_key[0]};
    }

    int64_t score(const Evaluation& evaluation) const {
        return evaluation.cost - evaluation.bonus;
    }

    template <class Emit>
    void expand(const Evaluation& evaluation, fast_beam::StateKey key,
                Emit&& emit) {
        constexpr int DIST_CAP = 6;
        constexpr int HORIZON_SCALE = 25000;
        constexpr int FUTURE_GAIN_WEIGHT = 2;
        constexpr int NEW_CELL_REWARD = 100;
        const auto [tx, ty] = problem_->targets[turn_];
        const int target = tx * problem_->n + ty;
        const int previous_pos = pos_;
        const int turn = turn_;
        const auto& next = problem_->next_pos[previous_pos];
        for (int a = 0; a < 3; ++a) {
            const int p = next[a];
            const bool is_new = visited_[p] == 0;
            fast_beam::StateKey next_key{
                key.low ^ problem_->pos_key[previous_pos].low ^ problem_->pos_key[p].low,
                key.high ^ problem_->pos_key[previous_pos].high ^ problem_->pos_key[p].high,
            };
            if (is_new) {
                next_key.low ^= problem_->cell_key[p].low;
                next_key.high ^= problem_->cell_key[p].high;
            }

            apply(Action(a));
            int64_t gain = 0;
            // お札によって距離が縮んだ将来の目標だけを加点する。
            // 遠い将来の目標ほど、後続のお札に代替されやすいので割り引く。
            for (size_t i = frames_.back(); i < changes_.size(); ++i) {
                const auto& change = changes_[i];
                const int deadline = problem_->target_time[change.cell];
                if (deadline <= turn) continue;
                const int old_dist = min<int>(change.old_distance, DIST_CAP);
                const int new_dist = min<int>(nearest_[change.cell], DIST_CAP);
                if (old_dist == new_dist) continue;
                const int horizon = HORIZON_SCALE /
                                    ((new_dist + 2) * (new_dist + 2));
                const int delay = deadline - turn;
                if (delay >= horizon) continue;
                gain += int64_t(old_dist - new_dist) *
                        int(problem_->weight[deadline]) * (horizon - delay) / horizon;
            }
            Evaluation updated{
                evaluation.cost + int(nearest_[target] * problem_->weight[turn]),
                evaluation.bonus + gain * FUTURE_GAIN_WEIGHT +
                    (is_new ? 1LL * NEW_CELL_REWARD *
                                  (problem_->total - turn) / problem_->total : 0),
            };
            emit(Action(a), updated, next_key, turn + 1 == problem_->total);
            undo(Action(a));
        }
    }

    void apply(const Action& action) {
        frames_.push_back(changes_.size());
        pos_ = problem_->next_pos[pos_][action];
        if (++visited_[pos_] == 1) {
            queue_.clear();
            relax(pos_, 0);
            for (size_t head = 0; head < queue_.size(); ++head) {
                const int cell = queue_[head];
                const uint8_t distance = nearest_[cell] + 1;
                for (const int neighbor : problem_->neighbors[cell]) {
                    if (neighbor >= 0) relax(neighbor, distance);
                }
            }
        }
        ++turn_;
    ...
    void undo(const Action& action) {
        --visited_[pos_];
        const size_t begin = frames_.back();
        while (changes_.size() > begin) {
            const auto change = changes_.back();
            nearest_[change.cell] = change.old_distance;
            changes_.pop_back();
        }
        frames_.pop_back();
        pos_ = problem_->prev_pos[pos_][action];
        --turn_;
    }
...
private:
    ...
    void relax(int cell, uint8_t distance) {
        if (distance >= nearest_[cell]) return;
        changes_.push_back({cell, nearest_[cell]});
        nearest_[cell] = distance;
        queue_.push_back(cell);
    }

    const Problem* problem_;
    vector<uint16_t> visited_;
    vector<uint8_t> nearest_;
...
int main() {
    ...
    int m;
    cin >> problem.n >> m;
    problem.total = problem.n * problem.n;
    problem.targets.resize(problem.total);
    for (auto& [x, y] : problem.targets) cin >> x >> y;
    prepare(problem);

    constexpr size_t BEAM_WIDTH = 4;
    auto result = fast_beam::fixed_beam_search(
        State(problem), {BEAM_WIDTH, problem.total});
    assert(result.finished && result.actions.size() == size_t(problem.total));

    for (const auto& move : problem.moves) {
        cout << move.dx << ' ' << move.dy << '\n';
    }
    for (auto action : result.actions) cout << int(action) << '\n';
}

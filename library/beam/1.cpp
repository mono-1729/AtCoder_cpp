...
public:
    ...
    pair<Evaluation, fast_beam::StateKey>
    initial_node() const {
        return {
            Evaluation{0},
            fast_beam::StateKey{0, 0},
        };
    }
    ...
    int64_t score(
        ...
        return target_ - evaluation.value;
    }

    template <class Emit>
    void expand(
        const Evaluation&,
        const fast_beam::StateKey&,
        Emit emit
    ) {
...
int main() {
    fast_beam::FixedBeamConfig config;

    config.beam_width = 2;
    config.max_turn = 10;

    auto result =
        fast_beam::fixed_beam_search(
            ToyState(7),
            config
        );
    ...
    for (const ToyState::Action& action :
         result.actions) {
        cout << " +" << action.add;
        total += action.add;
    }

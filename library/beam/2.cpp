...
int main() {
    ...
    constexpr int MAX_COST = 2;
    constexpr int MAX_TURN = 10;
    constexpr int BEAM_WIDTH = 2;

    using Selector =
        fast_beam::TopKSelector<Node>;

    using Candidate =
        fast_beam::Candidate<Node>;

    array<Selector, MAX_COST + 1> selectors{
        Selector(BEAM_WIDTH),
        Selector(BEAM_WIDTH),
        Selector(BEAM_WIDTH),
    };

    const array<Move, 2> moves{
        Move{1, 1},
        Move{3, 2},
    };

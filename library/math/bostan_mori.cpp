// https://github.com/drken1215/algorithm/blob/master/MathAlgebra/bostan_mori.cpp
...
(const FPS<mint> &P, const FPS<mint> &Q, long long N) {
    ...
    FPS<mint> S, T;
    if (N % 2 == 0) {
        for (int i = 0; i*2 < (int)P2.size(); ++i) S.emplace_back(P2[i*2]);
    } else {
        for (int i = 0; i*2+1 < (int)P2.size(); ++i) S.emplace_back(P2[i*2+1]);
    }
    for (int i = 0; i*2 < (int)Q2.size(); ++i) T.emplace_back(Q2[i*2]);

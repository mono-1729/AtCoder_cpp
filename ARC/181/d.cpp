...
int main() {
    ...
    vector<ll> a(m);
    rep(i,0,m) cin >> a[i];
    segtree<T, op2, e2> seg(n+1);
    ll sum = 0;
    LazySegmentTree<T, op2, e2, F, mapping, composition, id> lseg(m);
    rep(i,0,n){
        ll num = seg.prod(p[i],n+1);
        sum += num;
        ll l = lower_bound(a.begin(), a.end(), i+1) - a.begin();
        lseg.apply(l,min(m,l+num),1);
        seg.set(p[i],1);
    }
    rep(i,0,m){
        sum -= lseg.get(i);

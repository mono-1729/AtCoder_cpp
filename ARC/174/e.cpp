...
ll maxnum=300005;
vector<mint> fac(maxnum), inv(maxnum), finv(maxnum);
void init_fac(){
    fac[0] = fac[1] = 1;
    inv[0] = 0;
    inv[1] = 1;
...
int main() {
    ...
    LazySegmentTree<S,op,e,F,mapping,composition,id> use(vector<S>(n,{1,1})), ans1(vector<S>(k,{0,1})), ans2(vector<S>(n,{0,1}));
    vector<bool> check(n,false);
    vector<mint> ans(n); 
    rep(i,0,k){
        use.apply(p[i],-1);
        mint can = use.prod(0,p[i]).value;
        ans1.apply(0,i,can*fac[n-i-1]*finv[n-k]);
        ans2.apply(0,p[i],(can-1)*fac[n-i-1]*finv[n-k]*(k-i-1)*inv[n-i-1]+fac[n-i-1]*finv[n-k]);
        ans2.apply(p[i],n,can*fac[n-i-1]*finv[n-k]*(k-i-1)*inv[n-i-1]);
        ans[p[i]] += ans2.get(p[i]).value;
        check[p[i]] = true;
    }
    rep(i,0,k){
        ans[p[i]] += ans1.get(i).value;
        ans[p[i]]++;
    }
    rep(i,0,n) if(!check[i]) ans[i] += ans2.get(i).value;

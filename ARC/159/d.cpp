...
int main() {
    ...
    for(auto x:st) mp[x]=mp.size();
    SegTree<S> seg1(mp.size(),fx,ex),seg2(mp.size(),fx,ex);
    seg1.update(0,0);
    seg2.update(0,0);
    rep(i,0,n){
        ll num=max(seg1.query(0,mp[l[i]])+r[i]-l[i]+1,seg2.query(mp[l[i]],mp[r[i]])+r[i]);
        seg1.update(mp[r[i]],num);
        seg2.update(mp[r[i]],num-r[i]);
    }
    cout<<seg1.query(0,mp.size())<<endl;
    return 0;
}
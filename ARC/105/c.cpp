...
struct SegTree {
    ...
    void set(ll i, X x) { chmax(dat[i + n - 1],x); }
    void build() {
...
int main() {
    ...
    map<ll,ll> mp;
    for(auto i:s) mp[i]=mp.size()+1;
    SegTree<S> seg(mp.size()+1,fx,ex);
    rep(i,0,m) seg.set(mp[v[i]],l[i]);
    seg.build();
    ll len=INF;
    do{
        vector<ll> ans(n,0);
        rep(i,1,n){
            ...
            rep(j,0,i+1)sum+=w[j];
            rep(j,0,i){
                auto itr = mp.lower_bound(sum);
                if(itr==mp.begin())break;
                itr--;
                ll index = itr->second;
                chmax(ans[i],ans[j]+seg.query(0,index+1));
                sum-=w[j];
            }
        }
        chmin(len,ans.back());

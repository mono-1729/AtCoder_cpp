...
int main() {
    ...
    vector<ld> ans;
    rep(_,0,2){  
        sort(all(vec), [](tuple<ll,ll,ll> p, tuple<ll,ll,ll> q){
            auto [p1,p2,p3] = p;
            auto [q1,q2,q3] = q;
            ld num1 = -q2*p1, num2 = -q1*p2;
            if((p2 > 0) != (q2 > 0)) return num2 < num1;
            else return num1 < num2;
        });
        auto f = [&](ld mid) -> ll {
            ll sum = 0;
            vector<pair<ld,ll>> v(n);
            rep(i,0,n){
                auto [a,b,c] = vec[i];
                ld x = (c-a*mid)/((ld)b);
                v[i] = {x,i};
        ...
        ld ng = -INF, ok = INF;
        while((ok-ng)/max((ld)1.0,(ld)min(abs(ok), abs(ng))) > 1e-12){
            ld mid = (ng+ok)/2.0;
            if(f(mid)) ok = mid;
            else ng = mid;
        }

...
int main() {
    ll t; cin >> t;
    Matrix<mint> A(49,49);
    rep(i,0,49)rep(j,0,49)A[i][j] = 0;
    rep(i,7,49) A[i][i-7] = 1;
    rep(i,0,49){
        A[i][i] = 50;
        if(i%7 == 6) A[i][i]++;
        if(i%7 != 0) A[i][i-1] = 1;
    ...
    vector<Matrix<mint>> mvec(30);
    mvec[0]=A;
    // rep(i,0,49){
    //     rep(j,0,49) cout << mvec[0][i][j].val() << " ";
    //     cout << endl;
    // }
    rep(i,1,30) mvec[i] = mvec[i-1] * mvec[i-1];
    ...
    while(t--){
        ...
        dp[0][0] = 1;
        rep(i,0,30){
            if((n>>i)&1) dp = mvec[i]*dp;
        }
        cout << dp[48][0].val() << endl;
    }
    return 0;
}